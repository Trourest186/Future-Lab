...
# (CURL_TRIGGER_TIME, host, image, target_pods, repetition, WARM_CPU_TO_ACTIVE_PROCESS, "NONE")
# CURL_RESPONSE_TIME.format(quality, "{}"), host, image, target_pods, repetition, RESPOND_TIME_WARM_CPU, quality, quality)
...
def get_prometheus_values_and_update_job(host: str, image: str, target_pods: int, state: str, repetition: int):
    ...
    if host == 'jetson':
        ...
        voltage, current, energy, real_power, apparent_power, reactive_power, power_factor, frequency = 0,0,0,0,0,0,0,0
        real_power = 100000
...
#exec_pod(CURL_TERM, target_pod, "auto_delete"):
#(CURL_TRIGGER_TIME, host, image, target_pods, repetition, WARM_CPU_TO_ACTIVE_PROCESS, "NONE")
#(CURL_RESPONSE_TIME.format(quality, "{}"), host, image, target_pods, repetition, RESPOND_TIME_WARM_CPU, quality)
...
def connect_pod_exec(target_command: str, result_queue, lock, target_name: str = "ubuntu"):
    ...
    while trial < 20:
        try:
            output = subprocess.check_output(['/bin/bash', '-c', command]) #  # Khong cần fix
            with lock:
